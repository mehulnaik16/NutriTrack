...
for (const [group, names] of Object.entries(EXERCISES_DB)) {
  for (const n of names)
    if (!MUSCLE_OF.has(n.toLowerCase())) MUSCLE_OF.set(n.toLowerCase(), group);
}
...
const isCardio = (l: Log) =>
  !Array.isArray(l.exercises_done) && l.duration_min > 0;

/** Best estimated 1RM across a log's sets. */
const best1RM = (l: Log) =>
  setsOf(l).reduce(
    (b, s) => Math.max(b, estimate1RM(num(s.weight), num(s.reps))),
    0,
  );
...
export function WorkoutLogHistory() {
  ...
  const strengthLogs = useMemo(
    () => allLogs.filter((l) => setsOf(l).length > 0),
    [allLogs],
  );
  const loggedDates = useMemo(
    () => [...new Set(strengthLogs.map((l) => l.date))],
    [strengthLogs],
  );
  ...
  const priorBest = (l: Log) =>
    strengthLogs
      .filter(
        (x) => x.workout_name === l.workout_name && x.logged_at < l.logged_at,
      )
  ...
  const chartData = useMemo(() => {
    ...
    return strengthLogs
      ...
      .map((l) => ({
        ...
        maxWeight: round1(
          setsOf(l).reduce((m, s) => Math.max(m, setWeightIn(s, origUnit)), 0),
        ),
        e1rm: round1(
          setsOf(l).reduce(
            (b, s) =>
              Math.max(b, estimate1RM(setWeightIn(s, origUnit), num(s.reps))),
            0,
          ),
        ),
        volume: round1(
          setsOf(l).reduce((v, s) => {
            const w = setWeightIn(s, origUnit),
              r = num(s.reps);
            if (!w || !r) return v;
            return v + w * r;
          }, 0),
  ...
  return (
    <div className="space-y-8">
              ...
              >
                ...
                {isOpen &&
                  items.map((l, i) => {
                    ...
                    const volume = round1(
                      sets.reduce(
                        (v, s) => v + setWeightIn(s, weightUnit) * num(s.reps),
                        0,
                      ),
                    );
                    ...
                    const topOrig = sets.reduce(
                      (b, s) =>
                        Math.max(
                          b,
                          estimate1RM(setWeightIn(s, origUnit), num(s.reps)),
                        ),
                      0,
                    );
                    const top = round1(
                        ...
                        {open && (
                          <div className="mx-4 mb-4 rounded-xl border border-border/70 bg-muted/30 p-4">
                            ...
                            <div className="space-y-1.5 border-t border-border/60 pt-2">
                              ...
                              {sets.map((s, si) => {
                                const rm = estimate1RM(
                                  setWeightIn(s, origUnit),
                                  num(s.reps),
                                );
                                const isPR =
                                  rm > 0 && rm === topOrig && rm > prThreshold;
                                return (
                                  <div
                                    key={si}
      ...
      <div className="space-y-4">
        ...
        {cardioRows.length > 0 && (
          <div className="space-y-3">
            {cardioRows.map((l) => {
              const ex = !Array.isArray(l.exercises_done)
                ? l.exercises_done
                : null;
              const dist = ex?.distance
                ? parseFloat(String(ex.distance))
                : null;
                ...
                >
                  <div className="flex items-center gap-3 px-4 py-3.5">
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {l.duration_min} min · {Math.round(l.calories_burned)}{" "}
                        kcal
                        {l.confidence === "measured" && (
                          <span className="ml-1 text-[10px] font-semibold text-green-500">
                            ● HR
                          </span>
                        )}
                      </p>
                  ...
                  {open && (
                    <div className="mx-4 mb-4 rounded-xl border border-border/70 bg-muted/30 p-4">
                      <p className="mb-2 text-xs text-muted-foreground font-semibold">
                        {l.date}
                      </p>
      ...
      <Dialog open={calOpen} onOpenChange={setCalOpen}>
        <DialogContent className="max-w-sm">
          ...
          <Calendar
            mode="single"
            selected={
              activeDate ? new Date(activeDate + "T00:00:00") : undefined
            }
            onSelect={(d) => {
              if (!d) return;
      ...
      <Dialog
        open={!!cardioChartFor}
        onOpenChange={() => setCardioChartFor(null)}
      >
        <DialogContent className="max-w-lg">
          <DialogHeader>
