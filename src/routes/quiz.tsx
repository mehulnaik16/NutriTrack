...
function Quiz() {
  ...
  const goalKey = useMemo(
    () => resolveGoalKey(d.goal, loseRate),
    [d.goal, loseRate],
  );
  const target = useMemo(
    () => calcCalorieTarget(tdee, goalKey, d.gender),
    [tdee, goalKey, d.gender],
  );
  const macros = useMemo(
    () => calcMacros(target, goalKey, d.weightKg),
          ...
          {stepKey === "goal" && (
            <div className="space-y-6 animate-in fade-in duration-300">
              ...
              <RadioGroup
                ...
                onValueChange={(v) => {
                  set("goal", v);
                  setLoseRate(v === "gain" ? "gain_0_25kg" : "lose_0_25kg");
                }}
                className="grid gap-3"
              >
              ...
              {target > 0 && (
                <div className="rounded-xl border border-accent/30 bg-accent/10 p-4 mt-8">
                  ...
                  <div className="text-4xl font-bold text-center text-accent">
                    {target}{" "}
                    <span className="text-lg font-normal text-accent/70">
                      kcal
                    </span>
                  </div>
          ...
          {stepKey === "review" && (
            <div className="space-y-6 animate-in fade-in duration-300">
              ...
              <div className="grid gap-3 sm:grid-cols-2 text-sm">
                ...
                <Row
                  label="Weight"
                  value={`${unit === "kg" ? d.weightKg : Math.round(d.weightKg * 2.20462)} ${unit}`}
                />
...
function Row({
  ...
  return (
    ...
    >
      ...
      <span
        className={`font-medium ${highlight ? "text-accent" : "text-foreground"}`}
      >
        {value}
      </span>
    </div>
